...
(function () {
    document.addEventListener("DOMContentLoaded", () => {
        setupSocialAuth();
        updateNavbarUserSession(); // reflect any existing session on page load
    });
    ...
    function openSocialAuthModal(provider) {
        ...
        // ---------------- GOOGLE REALISTIC TEMPLATE ----------------
        const googleMarkup = `
            <div class="social-modal-backdrop"></div>
            <div class="gsi-modal-dialog">
                <button type="button" class="gsi-close-btn" aria-label="Close">&times;</button>

                <div class="gsi-header">
                    <div class="gsi-logo">${config.svg}</div>
                    <h2>Choose an account</h2>
                    <p>to continue to <strong>Techie-Tornadoes</strong></p>
                </div>

                <div class="gsi-account-list">
                    ${config.accounts.map((acc, index) => `
                        <label class="gsi-account-row ${index === 0 ? 'selected' : ''}">
                            <input type="radio" name="socialAcc" value="${index}" ${index === 0 ? 'checked' : ''}>
                            <div class="gsi-avatar">${acc.name.charAt(0)}</div>
                            <div class="gsi-acc-text">
                    ...
                    <button type="button" id="toggleCustomAccBtn" class="gsi-account-row gsi-add-account">
                        <div class="gsi-avatar gsi-avatar-plus">+</div>
                        <div class="gsi-acc-text">
                ...
                <div id="customAccForm" class="gsi-custom-fields" hidden>
                    <div class="gsi-input-group">
                ...
                ${isRegistrationPage ? `
                    <div class="gsi-event-note">
                        Registering for: <strong>${selectedEvent}</strong>
                    </div>
                ` : ''}

                <div class="gsi-footer-note">
                    To continue, Google will share your name, email address and profile picture with Techie-Tornadoes.
                </div>

                <div class="gsi-actions">
                    <button type="button" class="social-cancel-btn">Cancel</button>
                    <button type="button" class="social-confirm-btn gsi-confirm-btn">
                        <span id="confirmNameBtn">Continue</span>
                    </button>
                </div>
            </div>
        `;

        // ---------------- GENERIC TEMPLATE (LinkedIn / GitHub) ----------------
        const genericMarkup = `
            <div class="social-modal-backdrop"></div>
            <div class="social-modal-dialog">
                <div class="social-modal-header" style="border-top: 4px solid ${config.brandColor}">
                    <div class="provider-badge" style="background:${config.brandColor}">${config.svg}</div>
                    <div>
        ...
        modal.innerHTML = isGoogle ? googleMarkup : genericMarkup;

        document.body.appendChild(modal);
        document.body.classList.add("modal-open");
        requestAnimationFrame(() => modal.classList.add("active"));

        // ---------------- SHARED INTERACTIVITY (works for both templates) ----------------
        const radioName = isGoogle ? ".gsi-account-row" : ".account-option-item";
        ...
        radios.forEach((radio) => {
            radio.addEventListener("change", (e) => {
                modal.querySelectorAll(radioName).forEach((item) => item.classList.remove("selected"));
                radio.closest(radioName).classList.add("selected");
                const selectedAcc = config.accounts[e.target.value];
                if (selectedAcc && confirmNameSpan) {
                    confirmNameSpan.textContent = isGoogle ? "Continue" : selectedAcc.name;
                }
        ...
        customToggleBtn.addEventListener("click", () => {
            ...
            if (!isHidden) {
                ...
                if (selectedRadio) {
                    confirmNameSpan.textContent = isGoogle ? "Continue" : config.accounts[selectedRadio.value].name;
                }
            } else {
                confirmNameSpan.textContent = isGoogle ? "Continue" : "Custom User";
        ...
        const closeBtn = modal.querySelector(".social-close-btn, .gsi-close-btn");
        ...
        confirmBtn.addEventListener("click", () => {
            ...
            if (!customAccForm.hidden && customEmailInput && customEmailInput.value.trim()) {
                ...
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(finalEmail)) {
                    showTechieToast("⚠️ Please enter a valid email address.", true);
                    return;
                }
            } else {
                const selectedRadio = modal.querySelector('input[name="socialAcc"]:checked');
            ...
            const existingList = window.TechieStorage
                ? (window.TechieStorage.getRegistrations ? window.TechieStorage.getRegistrations() : [])
                : (JSON.parse(localStorage.getItem("techieTornadoesRegistrations")) || []);

            const alreadyExists = existingList.some((r) => r.email === finalEmail);
            ...
            showTechieToast(`🎉 ${provider} Authentication Successful! Signed in as ${finalName}`);
                ...
                formMessage.innerHTML = `
                    <div class="registration-success-badge">
                        ...
                        <div class="ticket-details">
                            <div class="ticket-row"><span class="t-label">Registration ID:</span><strong class="t-value id-code">${regId}</strong></div>
                            <div class="ticket-row"><span class="t-label">Candidate Name:</span><span class="t-value">${finalName}</span></div>
                            <div class="ticket-row"><span class="t-label">Registered Event:</span><strong class="t-value event-name">${selectedEvent}</strong></div>
                            <div class="ticket-row"><span class="t-label">Social Email:</span><span class="t-value">${finalEmail}</span></div>
                            <div class="ticket-row"><span class="t-label">Auth Provider:</span><span class="t-value">Verified via ${provider} OAuth</span></div>
                        </div>
            ...
            } else if (isLoginPage && formMessage) {
                formMessage.innerHTML = `
    ...
    // ---------------------------------------------------
    // Toast Notification (self-contained, no external file needed)
    // ---------------------------------------------------
    function showTechieToast(message, isError) {
        const existing = document.getElementById("techieToast");
        if (existing) existing.remove();

        const toast = document.createElement("div");
        toast.id = "techieToast";
        toast.className = "techie-toast" + (isError ? " toast-error" : "");
    ...
    window.showTechieToast = showTechieToast;

    // ---------------------------------------------------
    // Navbar Session Indicator (self-contained, no external file needed)
    // ---------------------------------------------------
    function updateNavbarUserSession() {
        const session = JSON.parse(localStorage.getItem("techie_user_session") || "null");
        const loginBtn = document.querySelector(".login-btn");
        if (!loginBtn) return;

        if (session && session.name) {
            loginBtn.innerHTML = `<span>👤</span> ${session.name.split(" ")[0]}`;
            loginBtn.setAttribute("href", "#");
            loginBtn.onclick = (e) => {
                e.preventDefault();
                if (confirm("Log out of Techie-Tornadoes?")) {
                    localStorage.removeItem("techie_user_session");
                    window.location.href = "index.html";
                }
            };
        }
    }
    window.updateNavbarUserSession = updateNavbarUserSession;
})();